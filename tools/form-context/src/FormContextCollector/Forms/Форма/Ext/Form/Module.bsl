...
Функция ОпределитьЗаголовокДляAI(Элемент)
	...
	Попытка
		Заголовок = СокрЛП(Строка(Элемент.Заголовок));
		Если Не ПустаяСтрока(Заголовок) Тогда
			Возврат Заголовок;
		КонецЕсли;
	Исключение
	...
	Попытка
		Заголовок = СокрЛП(Строка(Элемент.Подсказка));
		Если Не ПустаяСтрока(Заголовок) Тогда
			Возврат Заголовок;
		КонецЕсли;
	Исключение
	...
	Попытка
		...
		Если РасширеннаяПодсказка <> Неопределено Тогда
			Заголовок = СокрЛП(Строка(РасширеннаяПодсказка.Заголовок));
			Если Не ПустаяСтрока(Заголовок) Тогда
				Возврат Заголовок;
			КонецЕсли;
		КонецЕсли;
	...
	// Шаг 4: Используем имя элемента в крайнем случае
	Попытка
		Заголовок = СокрЛП(Строка(Элемент.Имя));
		Если Не ПустаяСтрока(Заголовок) Тогда
			Возврат Заголовок;
		КонецЕсли;
	Исключение
...
Функция СгенерироватьMarkdownНовыйФормат(Форма)
	...
	Исключение
		// Командная панель может отсутствовать
	КонецПопытки;
...
Функция СформироватьСтрокиЭлементаРекурсивно(Элемент, Глубина, ВыгружатьСкрытые, РодительЭлемент)
	...
	Результат = СтрШаблон("%1- [%2] ""%3"" (%4)%5",
		Отступ, Тег, Заголовок, ИмяЭлемента,
	...
	Если Не ПропускатьДетей Тогда
		Попытка
			...
			Для Каждого ДочернийЭлемент Из КоллекцияПодчиненных Цикл
				СтрокиДочернего = СформироватьСтрокиЭлементаРекурсивно(
					ДочернийЭлемент,
					Глубина + 1,
					ВыгружатьСкрытые,
					Элемент  // Передаем текущий элемент как родителя
				);
				Результат = Результат + СтрокиДочернего;
			КонецЦикла;
		Исключение
			// У элемента нет подчиненных элементов
		КонецПопытки;
	КонецЕсли;
