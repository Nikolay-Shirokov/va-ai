...
Процедура ОбновитьСписокФорм(Команда = Неопределено)
	...
	Попытка
		...
		Для Каждого ИнфоФормы Из МассивФорм Цикл
			...
			НоваяСтрока.СсылкаНаФорму = ИнфоФормы.СсылкаНаФорму;
		КонецЦикла;
...
Процедура СобратьКонтекст(Команда)
	...
	Попытка
		...
		Для Каждого ВыбраннаяФорма Из ВыбранныеФормы Цикл
			Попытка
				// Сбор данных формы
				ДанныеФормы = СобратьИнформациюОФорме(ВыбраннаяФорма.СсылкаНаФорму);
...
Функция ПолучитьСписокОткрытыхФорм()
	...
	Для Каждого Окно Из ВсеОкна Цикл
		Попытка
			...
			МассивФормОкна = Окно.Содержимое;
			
			Для Каждого Форма Из МассивФормОкна Цикл
				Попытка
					ИнфоФормы = Новый Структура;
					ИнфоФормы.Вставить("Заголовок", ПолучитьЗаголовокФормы(Форма));
					ИнфоФормы.Вставить("ИмяФормы", ПолучитьПолноеИмяФормы(Форма));
					ИнфоФормы.Вставить("ВидОбъекта", ПолучитьВидОбъекта(Форма));
					ИнфоФормы.Вставить("ТипФормы", ПолучитьТипФормы(Форма));
...
Функция СобратьИнформациюОФорме(Форма)
	...
	Метаданные.Вставить("collected_at", ТекущаяДатаСеанса());
