...
// Соответствие УИД формы → ссылка на форму (хранится в памяти клиента)
Перем СоответствиеФорм;
...
Процедура ПриОткрытии(Отказ)
	// Инициализация соответствия форм
...
Процедура ОбновитьСписокФорм(Команда = Неопределено)
	...
	Попытка
		...
		Для Каждого ИнфоФормы Из МассивФорм Цикл
			...
			// Сохраняем ссылку на форму в память клиента
			СоответствиеФорм.Вставить(ИнфоФормы.УИДФормы, ИнфоФормы.СсылкаНаФорму);
	...
	Попытка
		...
		Для Каждого ВыбраннаяФорма Из ВыбранныеФормы Цикл
			Попытка
				...
				// Получаем ссылку на форму по УИД
				СсылкаНаФорму = СоответствиеФорм.Получить(ВыбраннаяФорма.УИДФормы);
				Если СсылкаНаФорму = Неопределено Тогда
					ВызватьИсключение "Форма с УИД " + ВыбраннаяФорма.УИДФормы + " не найдена в памяти";
				КонецЕсли;
	...
	Для Каждого ТекущееОкно Из КоллекцияОкон Цикл
		Попытка
			...
			Для Каждого ТекущаяФорма Из МассивФормОкна Цикл
				Попытка
					// Генерируем уникальный идентификатор для формы
					УИДФормы = Строка(Новый УникальныйИдентификатор);
					...
					ИнфоФормы.Вставить("СсылкаНаФорму", ТекущаяФорма);
					
					МассивНайденныхФорм.Добавить(ИнфоФормы);
				Исключение
					// Пропускаем формы, которые не удалось обработать
				КонецПопытки;
			КонецЦикла;
		Исключение
			// Пропускаем окна, которые не удалось обработать
		КонецПопытки;
	КонецЦикла;
...
Функция ПолучитьЗаголовокФормы(Форма)
	Попытка
		Возврат Строка(Форма.Заголовок);
	Исключение
...
Функция ПолучитьПолноеИмяФормы(Форма)
	Попытка
		// Пытаемся получить имя формы через свойство ИмяФормы
		ИмяФормы = Строка(Форма.ИмяФормы);
		Если Не ПустаяСтрока(ИмяФормы) Тогда
			Возврат ИмяФормы;
		КонецЕсли;
	Исключение
		// Не удалось получить ИмяФормы
	КонецПопытки;
...
Функция ПолучитьВидОбъекта(Форма)
	Попытка
		...
		КонецЕсли;
	Исключение
		// Не удалось определить вид объекта
	КонецПопытки;
...
Функция ПолучитьТипФормы(Форма)
	Попытка
		...
		КонецЕсли;
	Исключение
		// Не удалось определить тип формы
	КонецПопытки;
...
Функция СобратьИнформациюОФорме(Форма)
	...
	Метаданные.Вставить("collected_at", ТекущаяДата());
...
Процедура СобратьЭлементыФормы(Форма, МассивЭлементов, ТекущаяГлубина)
	...
	Исключение
		// Игнорируем ошибки сбора элементов
	КонецПопытки;
КонецПроцедуры
