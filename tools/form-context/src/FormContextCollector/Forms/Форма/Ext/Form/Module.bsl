...
Функция СобратьИнформациюОФорме(Форма)
	...
	// Сбор элементов формы (передаем коллекцию Форма.Элементы)
	Попытка
		СобратьЭлементыФормы(Форма.Элементы, ДанныеФормы.elements, 0);
	Исключение
...
Функция СоздатьОписаниеЭлемента(Элемент, ТекущаяГлубина)
	...
	Попытка
		...
		Лог(СтрШаблон("      [Глубина %1] %2: найдено %3 подчиненных элементов",
			ТекущаяГлубина, ИмяЭлемента, КоличествоПодчиненных), "DEBUG");
		
		Если КоличествоПодчиненных > 0 Тогда
			МассивДочерних = Новый Массив;
			Лог(СтрШаблон("      [Глубина %1] %2: запуск рекурсии для подчиненных...", ТекущаяГлубина, ИмяЭлемента), "DEBUG");
			СобратьЭлементыФормы(ПодчиненныеЭлементы, МассивДочерних, ТекущаяГлубина + 1);
			...
			Иначе
				Лог(СтрШаблон("      [Глубина %1] %2: после рекурсии массив дочерних пуст (все пропущены)",
					ТекущаяГлубина, ИмяЭлемента), "DEBUG");
			КонецЕсли;
		КонецЕсли;
	Исключение
		// У элемента нет подчиненных - это нормально
		Лог(СтрШаблон("      [Глубина %1] %2: нет свойства ПодчиненныеЭлементы", ТекущаяГлубина, ИмяЭлемента), "DEBUG");
	КонецПопытки;
