...
Функция СобратьИнформациюОФорме(Форма)
	...
	// Дополнительно обрабатываем командную панель формы (AutoCommandBar)
	...
	Попытка
		...
		Если КоманднаяПанель <> Неопределено Тогда
			...
			// КоманднаяПанель - это ГруппаФормы, обрабатываем как обычный элемент
			ОписаниеКП = СоздатьОписаниеЭлемента(КоманднаяПанель, 0);
			Если ОписаниеКП <> Неопределено Тогда
				ДанныеФормы.elements.Добавить(ОписаниеКП);
				Лог("  ✓ Командная панель добавлена", "DEBUG");
			КонецЕсли;
		КонецЕсли;
	Исключение
		Лог("  Ошибка обработки командной панели: " + ОписаниеОшибки(), "DEBUG");
	КонецПопытки;
...
Функция СоздатьОписаниеЭлемента(Элемент, ТекущаяГлубина)
	...
	Если ТипЭлемента = "group" Или ТипЭлемента = "table" Тогда
		...
		Попытка
			...
			Если КоманднаяПанель <> Неопределено Тогда
				Лог(СтрШаблон("      [Глубина %1] %2: обработка командной панели", ТекущаяГлубина, ИмяЭлемента), "DEBUG");
				
				// КоманднаяПанель - это ГруппаФормы, добавляем её подчиненные элементы
				Попытка
					КоллекцияКП = КоманднаяПанель.ПодчиненныеЭлементы;
					Если КоллекцияКП.Количество() > 0 Тогда
						Лог(СтрШаблон("      [Глубина %1] %2: запуск рекурсии (КоманднаяПанель)...", ТекущаяГлубина, ИмяЭлемента), "DEBUG");
			...
			КонецЕсли;
		Исключение
			// У элемента нет командной панели - это нормально
			// Не логируем, т.к. это обычная ситуация для многих элементов
