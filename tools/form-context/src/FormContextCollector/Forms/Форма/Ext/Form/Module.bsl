		...
		Попытка
			...
			Для Каждого ТекущаяФорма Из МассивФормОкна Цикл
				Попытка
					...
					// Пропускаем саму форму обработки
					Если СтрНайти(ИнфоФормы.ИмяФормы, "FormContextCollector") > 0 Тогда
						Лог("  Пропущена форма самой обработки: " + ИнфоФормы.ИмяФормы, "DEBUG");
						Продолжить;
					КонецЕсли;
