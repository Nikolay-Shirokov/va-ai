...
Функция СоздатьОписаниеЭлемента(Элемент, ТекущаяГлубина)
	...
	// Используем другое имя переменной, чтобы избежать конфликта с свойством формы
	Если ТипЭлемента = "group" Или ТипЭлемента = "table" Тогда
		Попытка
			...
			Если КоличествоПодчиненных > 0 Тогда
				...
				Лог(СтрШаблон("      [Глубина %1] %2: запуск рекурсии...", ТекущаяГлубина, ИмяЭлемента), "DEBUG");
				...
				Иначе
					Лог(СтрШаблон("      [Глубина %1] %2: массив дочерних пуст после рекурсии",
