...
&НаКлиенте
Перем МенеджерVA;             // Ссылка на объект Vanessa Automation
&НаКлиенте
...
Процедура ПриОткрытии(Отказ)
	// Обработка может быть открыта двумя способами:
	// 1. Программно с вызовом ИнициализацияФормы(СсылкаНаVA)
	// 2. Вручную - тогда покажем инструкцию
...
// Показывает инструкцию по использованию обработки
//
&НаКлиенте
Процедура ПоказатьИнструкцию()
	
	Инструкция =
	"Эта обработка должна быть открыта из Vanessa Automation." + Символы.ПС + Символы.ПС +
	"Способы открытия:" + Символы.ПС + Символы.ПС +
	"1. Добавить в модуль формы VA:" + Символы.ПС +
	"   Обработка = ВнешниеОбработки.Создать(""путь/к/FormContextCollector.epf"");" + Символы.ПС +
	"   Форма = Обработка.ПолучитьФорму(""Форма"", ЭтотОбъект);" + Символы.ПС +
	"   Форма.ИнициализацияФормы(ЭтотОбъект);" + Символы.ПС +
	"   Форма.Открыть();" + Символы.ПС + Символы.ПС +
	"2. Или добавить кнопку в форму VA с этим кодом.";
	
	СтатусСбора = "Требуется инициализация";
	РезультатСбора = Инструкция;
	
КонецПроцедуры

// Инициализирует обработку со ссылкой на Vanessa Automation
// ЭКСПОРТНАЯ функция - вызывается из кода VA при открытии
//
// Параметры:
//   ВладелецФормы - Форма - Ссылка на форму Vanessa Automation
//
&НаКлиенте
Функция ИнициализацияФормы(ВладелецФормы) Экспорт
	МенеджерVA = ВладелецФормы;
	СтатусСбора = "Инициализация выполнена. Готов к работе.";
	РезультатСбора = "";
	Возврат Истина;
КонецФункции
...
Функция ИнициализироватьТестируемуюФорму()
	...
	Попытка
		// Подключаемся к TestClient, если еще не подключены
		Если ТестовоеПриложение = Неопределено Тогда
			ПодключитьсяКTestClient();
		КонецЕсли;
		
		// Получаем активное окно из TestClient
		Попытка
			// Для V83C.Application используется метод получения активного окна
			ТестируемоеОкно = ТестовоеПриложение.ActiveWindow;
		
		Исключение
			// Если ActiveWindow не работает, пробуем через FindObject
			ВсеОкна = ТестовоеПриложение.FindObjects(Тип("ТестируемоеОкноКлиентскогоПриложения"));
			Если ВсеОкна.Количество() > 0 Тогда
				// Берем последнее открытое окно
				ТестируемоеОкно = ВсеОкна[ВсеОкна.Количество() - 1];
			Иначе
				ВызватьИсключение "Не найдены открытые окна в клиенте тестирования.";
			КонецЕсли;
		КонецПопытки;
