...
// Путь к файлу лога (инициализируется при создании формы)
&НаКлиенте
Перем ПутьКФайлуЛога;

// Путь к файлу task.json для агентского режима (инициализируется при создании формы)
&НаКлиенте
Перем ПутьКФайлуTaskJSON;
...
Асинх Процедура ПриОткрытии(Отказ)
	// Инициализация на сервере (установка путей)
	СтруктураПутей = ИнициализацияПриОткрытииНаСервере();
	
	// Сохраняем пути в переменных модуля для клиентских методов
	ПутьКФайлуЛога = СтруктураПутей.ПутьКЛогу;
	ПутьКФайлуTaskJSON = СтруктураПутей.ПутьКTaskJSON;
	
	// Очищаем лог при каждом открытии (на клиенте)
	...
	РежимРаботы = Ждать ПроверитьРежимЗапуска();
...
&НаСервере
Функция ИнициализацияПриОткрытииНаСервере()
	// Определяем путь сохранения
	Объект.ПутьСохранения = ПолучитьПутьКонтекста();
	
	// Получаем базовый путь к обработке
	...
	// Формируем и возвращаем структуру с путями
	СтруктураПутей = Новый Структура;
	СтруктураПутей.Вставить("ПутьКЛогу", КаталогОбработки + "debug.log");
	СтруктураПутей.Вставить("ПутьКTaskJSON", КаталогОбработки + "agent" + ПолучитьРазделительПути() + "task.json");
	
	Возврат СтруктураПутей;
КонецФункции
...
&НаКлиенте
Асинх Процедура СобратьКонтекст(Команда)
	...
	Попытка
		...
		Для Каждого ВыбраннаяФорма Из ВыбранныеФормы Цикл
			Попытка
				...
				// Генерация и сохранение JSON (если включено)
...
Функция ПолучитьВидОбъекта(Форма)
	Попытка
		ИмяПроверяемойФормы = Строка(Форма.ИмяФормы);
		
		Если СтрНайти(ИмяПроверяемойФормы, "Документ.") = 1 Тогда
			Возврат "Документ";
		ИначеЕсли СтрНайти(ИмяПроверяемойФормы, "Справочник.") = 1 Тогда
			Возврат "Справочник";
		ИначеЕсли СтрНайти(ИмяПроверяемойФормы, "Отчет.") = 1 Тогда
			Возврат "Отчет";
		ИначеЕсли СтрНайти(ИмяПроверяемойФормы, "Обработка.") = 1 Тогда
			Возврат "Обработка";
		ИначеЕсли СтрНайти(ИмяПроверяемойФормы, "РегистрСведений.") = 1 Тогда
			Возврат "РегистрСведений";
		ИначеЕсли СтрНайти(ИмяПроверяемойФормы, "РегистрНакопления.") = 1 Тогда
			Возврат "РегистрНакопления";
		ИначеЕсли СтрНайти(ИмяПроверяемойФормы, "ОбщаяФорма.") = 1 Тогда
			Возврат "ОбщаяФорма";
		КонецЕсли;
	Исключение
...
Функция ПолучитьТипФормы(Форма)
	Попытка
		ИмяАнализируемойФормы = Строка(Форма.ИмяФормы);
		
		// Извлекаем последнюю часть после последней точки
		МассивЧастей = СтрРазделить(ИмяАнализируемойФормы, ".", Ложь);
...
Функция СобратьИнформациюОФорме(Форма)
	...
	//BSLLS-off
	СлужебнаяИнформация.Вставить("collected_at", ТекущаяДата());
	//BSLLS-on
...
//
&НаКлиенте
Функция ОпределитьТипФормыПоИмени(ПолноеИмяФормы)
	Если СтрНайти(ПолноеИмяФормы, "Документ.") > 0 Тогда
		Возврат "document_form";
	ИначеЕсли СтрНайти(ПолноеИмяФормы, "Справочник.") > 0 Тогда
		Возврат "catalog_form";
	ИначеЕсли СтрНайти(ПолноеИмяФормы, "Обработка.") > 0 Тогда
		Возврат "data_processor_form";
	ИначеЕсли СтрНайти(ПолноеИмяФормы, "Отчет.") > 0 Тогда
		Возврат "report_form";
	ИначеЕсли СтрНайти(ПолноеИмяФормы, "РегистрСведений.") > 0 Тогда
...
Функция СоздатьОписаниеЭлемента(Элемент, ТекущаяГлубина)
	...
	ЗаголовокЭлемента = ПолучитьСвойствоЭлемента(Элемент, "Заголовок");
	Если Не ПустаяСтрока(ЗаголовокЭлемента) Тогда
		Описание.Вставить("title", ЗаголовокЭлемента);
	КонецЕсли;
...
Функция СформироватьСтрокиЭлементаРекурсивно(Элемент, Глубина, ВыгружатьСкрытые, РодительЭлемент)
	...
	Попытка
		ВидимостьЭлемента = Элемент.Видимость;
...
Асинх Процедура ОбновитьИндексФорм(ДанныеФормы, ИмяФайлаJSON)
	...
	Если Объект.ГенерироватьMarkdown Тогда
		ИмяФайлаMD = СтрЗаменить(?(ПустаяСтрока(ИмяФайлаJSON), 
			СформироватьИмяФайла(ДанныеФормы.form_name) + ".md",
			СтрЗаменить(ИмяФайлаJSON, ".json", ".md")), ".json", ".md");
	...
	//BSLLS-off
	Индекс.Вставить("updated_at", ТекущаяДата());
	//BSLLS-on
...
//
&НаКлиенте
Асинх Функция ПроверитьРежимЗапуска()
	Попытка
		ФайлTask = Новый Файл(ПутьКФайлуTaskJSON);
		Существует = Ждать ФайлTask.СуществуетАсинх();
...
//
&НаКлиенте
Асинх Функция ПрочитатьTaskJSON()
	// Читаем файл .processing (он уже должен быть переименован)
	ПутьProcessing = ПутьКФайлуTaskJSON + ".processing";
	ФайлProcessing = Новый Файл(ПутьProcessing);
	Существует = Ждать ФайлProcessing.СуществуетАсинх();
...
//
&НаКлиенте
Асинх Функция ПолучитьЗаданиеАгента()
	Возврат Ждать ПрочитатьTaskJSON();
КонецФункции
...
//
&НаКлиенте
Асинх Функция ПереименоватьTaskJSON(НовыйСтатус)
	Если НовыйСтатус = "processing" Тогда
		// task.json → task.json.processing
		ИсходныйФайл = ПутьКФайлуTaskJSON;
		НовыйФайл = ПутьКФайлуTaskJSON + ".processing";
		
		ФайлИсходный = Новый Файл(ИсходныйФайл);
		Существует = Ждать ФайлИсходный.СуществуетАсинх();
		Если Не Существует Тогда
	...
	Иначе
		...
		ИсходныйФайл = ПутьКФайлуTaskJSON + ".processing";
		НовыйФайл = ПутьКФайлуTaskJSON + "." + НовыйСтатус;
		...
		СуществуетУдаляемый = Ждать ФайлУдаляемый.СуществуетАсинх();
		Если СуществуетУдаляемый Тогда
			Ждать УдалитьФайлыАсинх(НовыйФайл);
		КонецЕсли;
	КонецЕсли;
	
	// Переименовываем файл
	Ждать ПереместитьФайлАсинх(ИсходныйФайл, НовыйФайл);
	Возврат Истина;
КонецФункции
...
//
&НаКлиенте
Асинх Функция ОбновитьСтатусTaskJSON(Статус)
	Возврат Ждать ПереименоватьTaskJSON(Статус);
КонецФункции

// Выполняет агентский режим (автоматический сбор)
//
&НаКлиенте
Асинх Процедура ВыполнитьАгентскийРежим()
	...
	Попытка
		...
		Ждать ОбновитьСтатусTaskJSON("processing");
		
		// 2. Читаем задание
		Лог("Чтение задания из task.json.processing");
		Задание = Ждать ПолучитьЗаданиеАгента();
		...
		Если МассивФормДляОбработки.Количество() = 0 Тогда
			...
			Ждать ОбновитьСтатусTaskJSON("completed");
	...
	Исключение
		...
		// Помечаем как ошибку
		Попытка
			Ждать ОбновитьСтатусTaskJSON("error");
		Исключение
...
//
&НаКлиенте
Асинх Процедура ОбработатьСледующуюФормуАгента(МассивФорм, Индекс, Задание)
...
//
&НаКлиенте
Асинх Процедура ПослеОткрытияФормыАгента(УИДФормы, ДополнительныеПараметры) Экспорт
...
//
&НаКлиенте
Асинх Процедура СобратьКонтекстОткрытойФормыАгента()
	...
	Попытка
		...
		// Генерируем и сохраняем JSON (если включено)
		ИмяФайлаJSON = "";
		Если Объект.ГенерироватьJSON Тогда
			Лог("Генерация JSON...");
			ТекстJSON = ПреобразоватьВJSON(ДанныеФормы);
...
//
&НаКлиенте
Асинх Процедура ЗавершитьАгентскийРежим(Задание, Статус)
	Лог("");
	...
	Попытка
		Ждать ОбновитьСтатусTaskJSON(Статус);
...
Процедура ПрименитьНастройкиЗадания(Опции)
	...
	Если Опции.Свойство("debug_mode") Тогда
		Объект.РежимОтладки = Опции.debug_mode;
		Лог(СтрШаблон("  - Режим отладки: %1", Опции.debug_mode));
	КонецЕсли;
...
Процедура ОткрытьФормуПрограммно(ОписаниеФормы, ОповещениеОЗавершении)
	...
	ИначеЕсли ОписаниеФормы.type = "metadata_object" Тогда
		...
		ИмяФормыОбъекта = ?(ОписаниеФормы.Свойство("form_name"),
			ОписаниеФормы.form_name, "ФормаОбъекта");
		ПутьКФорме = ОписаниеФормы.value + ".Форма." + ИмяФормыОбъекта;
		Лог(СтрШаблон("  Объект: %1, Форма: %2 → %3",
			ОписаниеФормы.value, ИмяФормыОбъекта, ПутьКФорме));
...
Процедура Лог(Сообщение, Уровень = "INFO")
	...
	Попытка
		...
		//BSLLS-off
		ВремяМетка = Формат(ТекущаяДата(), "ДФ='yyyy-MM-dd HH:mm:ss'");
		//BSLLS-on
		СтрокаЛога = СтрШаблон("[%1] [%2] %3", ВремяМетка, Уровень, Сообщение) + Символы.ПС;
		
		// Добавляем в файл (используем переменную модуля)
		ЗаписьТекста = Новый ЗаписьТекста(ПутьКФайлуЛога, КодировкаТекста.UTF8, , Истина);
...
//
&НаКлиенте
Асинх Процедура ОчиститьЛог()
	Попытка
		ФайлЛога = Новый Файл(ПутьКФайлуЛога);
		Существует = Ждать ФайлЛога.СуществуетАсинх();
		Если Существует Тогда
			Ждать УдалитьФайлыАсинх(ПутьКФайлуЛога);
		КонецЕсли;
	Исключение
