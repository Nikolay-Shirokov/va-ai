...
Функция ПолучитьЗаголовокФормы(Форма)
	Попытка
		// Пробуем разные свойства для получения заголовка
		ЗаголовокФормы = "";
		
		// Способ 1: через Заголовок
		Попытка
			ЗаголовокФормы = Строка(Форма.Заголовок);
			Лог("    Заголовок (Форма.Заголовок): '" + ЗаголовокФормы + "'", "DEBUG");
		Исключение
			Лог("    Ошибка Форма.Заголовок: " + ОписаниеОшибки(), "DEBUG");
		КонецПопытки;
		
		// Способ 2: через ТекущийЭлемент или ЗаголовокОкна
		Если ПустаяСтрока(ЗаголовокФормы) Тогда
			Попытка
				ЗаголовокФормы = Строка(Форма);
				Лог("    Заголовок (Строка(Форма)): '" + ЗаголовокФормы + "'", "DEBUG");
			Исключение
				Лог("    Ошибка Строка(Форма): " + ОписаниеОшибки(), "DEBUG");
			КонецПопытки;
		КонецЕсли;
		...
		КонецЕсли;
	Исключение
		Лог("    Критическая ошибка получения заголовка: " + ОписаниеОшибки(), "DEBUG");
		Возврат "Без заголовка";
	КонецПопытки;
КонецФункции
...
Функция ПолучитьПолноеИмяФормы(Форма)
	...
	// Способ 1: через свойство ИмяФормы
	Попытка
		ПолноеИмя = Форма.ИмяФормы;
		Лог("    ИмяФормы (Форма.ИмяФормы): '" + ПолноеИмя + "'", "DEBUG");
		Если Не ПустаяСтрока(ПолноеИмя) Тогда
			Возврат ПолноеИмя;
		КонецЕсли;
	Исключение
		Лог("    Ошибка Форма.ИмяФормы: " + ОписаниеОшибки(), "DEBUG");
	КонецПопытки;
	
	// Способ 2: через ИмяМетаданных
	Попытка
		ПолноеИмя = Форма.ИмяМетаданных;
		Лог("    ИмяМетаданных (Форма.ИмяМетаданных): '" + ПолноеИмя + "'", "DEBUG");
		Если Не ПустаяСтрока(ПолноеИмя) Тогда
			Возврат ПолноеИмя;
		КонецЕсли;
	Исключение
		Лог("    Ошибка Форма.ИмяМетаданных: " + ОписаниеОшибки(), "DEBUG");
	КонецПопытки;
	
	// Способ 3: через ИмяОбъектаМетаданных
	Попытка
		ПолноеИмя = Форма.ИмяОбъектаМетаданных;
		Лог("    ИмяОбъектаМетаданных (Форма.ИмяОбъектаМетаданных): '" + ПолноеИмя + "'", "DEBUG");
		Если Не ПустаяСтрока(ПолноеИмя) Тогда
			Возврат ПолноеИмя;
		КонецЕсли;
	Исключение
		Лог("    Ошибка Форма.ИмяОбъектаМетаданных: " + ОписаниеОшибки(), "DEBUG");
	КонецПопытки;
	
	// Если не удалось получить имя, возвращаем заголовок
	Лог("    Не удалось получить имя формы, используем заголовок", "DEBUG");
