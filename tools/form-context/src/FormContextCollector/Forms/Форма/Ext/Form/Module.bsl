...
Функция ОпределитьТипЭлемента(Элемент)
	Попытка
		...
		// Логируем тип для отладки (только на первом запуске)
		//Лог("    Тип элемента: " + ИмяТипа, "DEBUG");
		
		// Поля ввода: "Поле формы" (с пробелом!)
		Если СтрНайти(ИмяТипа, "Поле") > 0
			ИЛИ СтрНайти(ИмяТипа, "InputField") > 0
			ИЛИ СтрНайти(ИмяТипа, "CheckBoxField") > 0 Тогда
			Возврат "input_field";
			
		// Кнопки: "Кнопка формы"
		ИначеЕсли СтрНайти(ИмяТипа, "Кнопка") > 0
			ИЛИ СтрНайти(ИмяТипа, "Button") > 0 Тогда
			Возврат "button";
			
		// Таблицы: "Таблица формы"
		ИначеЕсли СтрНайти(ИмяТипа, "Таблица") > 0
			ИЛИ СтрНайти(ИмяТипа, "Table") > 0 Тогда
			Возврат "table";
			
		// Группы: "Группа формы"
		ИначеЕсли СтрНайти(ИмяТипа, "Группа") > 0
			ИЛИ СтрНайти(ИмяТипа, "Group") > 0
			ИЛИ СтрНайти(ИмяТипа, "CommandBar") > 0
			ИЛИ СтрНайти(ИмяТипа, "КоманднаяПанель") > 0 Тогда
			Возврат "group";
			
		// Надписи и декорации
		ИначеЕсли СтрНайти(ИмяТипа, "Надпись") > 0
			ИЛИ СтрНайти(ИмяТипа, "Декорация") > 0
			ИЛИ СтрНайти(ИмяТипа, "Label") > 0
			ИЛИ СтрНайти(ИмяТипа, "Decoration") > 0 Тогда
			Возврат "label";
		Иначе
			// Все остальные элементы (контекстные меню, дополнения и т.д.)
			Возврат "other";
		КонецЕсли;
	Исключение
		Лог("    Ошибка определения типа: " + ОписаниеОшибки(), "DEBUG");
		Возврат "unknown";
	КонецПопытки;
КонецФункции
