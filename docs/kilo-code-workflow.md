...
| Критерий | 🔴 Сверх-экономная | 🟢 Экономная | 🟡 Рекомендуемая | 🔵 Расширенная |
|----------|-------------------|-------------|------------------|----------------|
| **Токены контекста** | ~30-60K | ~50-100K | ~150-200K | ~250-300K |
| **Файлы в контексте** | `guide.md`<br>`templates.md` | `steps-library.json` | `steps-library.json`<br>`guide.md` | `steps-library.json`<br>`guide.md`<br>`templates.md`<br>`semantic-validation-guide.md` |
| **Точность AI (1 попытка)** | 60-70% | 75-80% | 90-95% | 95-98% |
| **Итераций с валидатором** | 3-5 | 2-3 | 1-2 | 1 |
| **Подходит для** | Прототипы<br>Критичный бюджет | Простые сценарии<br>Опытные пользователи | Большинство задач<br>Смешанный уровень ⭐ | Сложные сценарии<br>Обучение новичков |
| **Стоимость** | Минимальная | Низкая | Средняя | Высокая |
| **Время генерации** | Очень быстро | Быстро | Средне | Медленнее |
| **Потребность в правках** | Высокая (~40%) | Высокая (~30%) | Средняя (~10%) | Низкая (~5%) |
| **Валидатор** | ⚠️ ОБЯЗАТЕЛЕН | Рекомендован | Рекомендован | Опционален |
