...
| Критерий | 🔴 Сверх-экономная | 🟢 Экономная | 🟡 Рекомендуемая | 🔵 Расширенная | 🌟 Гибридная |
|----------|-------------------|-------------|------------------|----------------|--------------|
| **Токены контекста** | ~30-60K | ~50-100K | ~150-200K | ~250-300K | ~70-80K |
| **Файлы в контексте** | `guide.md`<br>`templates.md` | `steps-library.json` | `steps-library.json`<br>`guide.md` | `steps-library.json`<br>`guide.md`<br>`templates.md`<br>`semantic-validation-guide.md` | `guide.md`<br>`templates.md`<br>+search-steps.py |
| **Точность AI (1 попытка)** | 60-70% | 75-80% | 90-95% | 95-98% | 85-90% |
| **Итераций с валидатором** | 3-5 | 2-3 | 1-2 | 1 | 1-2 |
| **Подходит для** | Прототипы<br>Критичный бюджет | Простые сценарии<br>Опытные пользователи | Большинство задач<br>Смешанный уровень ⭐ | Сложные сценарии<br>Обучение новичков | Короткие сессии<br>1-10 сценариев 🌟 |
| **Стоимость** | Минимальная | Низкая | Средняя | Высокая | Низкая |
| **Время генерации** | Очень быстро | Быстро | Средне | Медленнее | Быстро |
| **Потребность в правках** | Высокая (~40%) | Высокая (~30%) | Средняя (~10%) | Низкая (~5%) | Средняя (~15%) |
| **Валидатор** | ⚠️ ОБЯЗАТЕЛЕН | Рекомендован | Рекомендован | Опционален | Рекомендован |
| **Статус** | ✅ Готов | ✅ Готов | ✅ Готов | ✅ Готов | ✅ Готов |

### ⚠️ Особенности сверх-экономной стратегии

**Принцип работы:**
AI генерирует сценарий на основе примеров из `guide.md` и `templates.md`, затем валидатор находит ошибки и предлагает правильные шаги из полной библиотеки. AI исправляет по рекомендациям.

**Плюсы:**
- ✅ Экономия ~50-60% токенов (58 KB вместо 655 KB)
- ✅ Понимание синтаксиса и логики из guide.md
- ✅ Готовые паттерны из templates.md
- ✅ Быстрая инициализация

**Минусы:**
- ❌ AI не знает точный синтаксис ~1400 шагов (знает только примеры)
- ❌ Может "додумывать" несуществующие варианты шагов
- ❌ Больше итераций исправлений (3-5 вместо 1-2)
- ❌ Валидатор становится критически важным

**Когда использовать:**
- 💰 Очень ограниченный бюджет на токены
- 🚀 Быстрое прототипирование
- 📚 Обучение (AI объясняет на примерах)
...
**Когда использовать:** Критичный бюджет, прототипирование, обучение на примерах
...
БАЗА ЗНАНИЙ:
- `.va-ai/knowledge/guide.md` - синтаксис, примеры шагов (~50-100 примеров из 1569)
- `.va-ai/knowledge/templates.md` - готовые шаблоны сценариев

⚠️ ВАЖНО: У тебя НЕТ полной библиотеки шагов!
- В guide.md есть только примеры основных шагов
- Валидатор подскажет правильный синтаксис при ошибках
- Генерируй на основе примеров, валидатор поправит детали

КРИТИЧЕСКИ ВАЖНО:
1. ✅ Следуй синтаксису и структуре из guide.md
2. ✅ Используй паттерны из templates.md как основу
3. ✅ Всегда используй двойные кавычки ""
4. ✅ Обязательные заголовки:
   # encoding: utf-8
   # language: ru
5. ✅ Структура: Функционал → Сценарий → Шаги (Дано/Когда/Тогда)

WORKFLOW (ОБЯЗАТЕЛЬНЫЙ ЦИКЛ):
1. Генерирую сценарий на основе примеров из guide.md и templates.md
2. ОБЯЗАТЕЛЬНО запускаю валидатор:
   python ../va-ai/tools/validator/validate.py <файл> --ai-enhanced
3. Валидатор проверит по ПОЛНОЙ библиотеке и найдет ошибки
4. Исправляю согласно рекомендациям валидатора:
   - auto_fix → применяю автоматически
   - semantic_check_required → проверяю семантику, применяю при совпадении
   - critical / low confidence → спрашиваю пользователя
5. Повторяю до 5 итераций (больше чем обычно!)
6. Если не помогло → детальный запрос к пользователю

⚠️ ОЖИДАНИЯ:
- Первая попытка: ~60-70% валидных шагов
- 3-5 итераций с валидатором до полной валидности
- Валидатор - ключевой инструмент исправления

Готов работать в режиме экономии!
```

**Команда для загрузки контекста:**
```
Kilo Code, прочитай файлы для понимания синтаксиса Vanessa Automation:
1. `.va-ai/knowledge/guide.md` - руководство с примерами шагов
2. `.va-ai/knowledge/templates.md` - готовые шаблоны

Это минимальный контекст. Валидатор подскажет точный синтаксис шагов.
```
